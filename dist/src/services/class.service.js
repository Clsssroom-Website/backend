...
export const updateClass = async (teacherId, classId, data) => {
    const existingClass = await ClassRepo.findClassById(classId);
    if (!existingClass) {
        throw new NotFoundError("Không tìm thấy lớp học.");
    }
...
export const deleteClass = async (teacherId, classId) => {
    const existingClass = await ClassRepo.findClassById(classId);
    if (!existingClass) {
        throw new NotFoundError("Không tìm thấy lớp học.");
    }
...
export const joinClass = async (studentId, codeOrLink) => {
...
export const getClassStream = async (classId) => {
    ...
    const processedAssignments = await Promise.all(assignments.map(async (a) => {
        let attachments = [];
        if (a.AssignmentAttachments?.length > 0) {
            attachments = await Promise.all(a.AssignmentAttachments.map(async (att) => {
    ...
    const processedDocuments = await Promise.all(documents.map(async (d) => {
        ...
        if (d.DocumentAttachments?.length > 0) {
            attachments = await Promise.all(d.DocumentAttachments.map(async (att) => {
