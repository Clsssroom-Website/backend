...
export class DocumentService {
    ...
    async uploadDocument(userId, classId, title, description, files) {
    ...
    async getDocumentsByClassId(userId, classId) {
    ...
    async getAttachmentDownloadUrl(userId, attachmentId, action) {
        ...
        if (isManagingAttachments) {
            ...
            for (const old of oldAttachments) {
                if (!keepIds.includes(old.attachmentId) && old.fileUri) {
                    await this.storageService.deleteFile(old.fileUri);
                }
            }
    ...
    async deleteDocument(userId, documentId) {
        ...
        for (const att of document.DocumentAttachments || []) {
            if (att.fileUri) {
                await this.storageService.deleteFile(att.fileUri);
            }
        }
        await this.documentRepo.deleteAllAttachments(documentId);
